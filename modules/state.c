...
struct state {
	...
	struct state_sounds sounds ;	// Ήχοι που πρέπει να αναπαραχθούν σε κάθε κατάσταση
};
...
static void add_objects(State state, float start_x) {
	...
	for (int i = 0; i < PLATFORM_NUM; i++) {
		...
		lastplatform  = vector_node_value(state->objects, vector_last(state->objects));
...
void state_update(State state, KeyState keys) {
	...
	if(state->info.playing && !state->info.paused){
		...
		if(state->info.ball->vert_mov == IDLE){
			for(int i = 0; i < vector_size(state->objects); i++){
			Object object = vector_get_at(state->objects, i);
				if(object != NULL && object->type == PLATFORM){
					if(state_info(state)->ball->rect.x + state_info(state)->ball->rect.width >= object->rect.x && state_info(state)->ball->rect.x <= object->rect.x + object->rect.width){
			...
			for(int i = 0; i < vector_size(state->objects); i++){
				Object object = vector_get_at(state->objects, i);
				if(object != NULL && object->type == PLATFORM){
					if(CheckCollisionRecs(state->info.ball->rect, object->rect) && state->info.ball->rect.y + state->info.ball->rect.height <= object->rect.y + object->rect.height/2){
						if(object->unstable){
